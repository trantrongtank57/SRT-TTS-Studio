...
# Onefile build — produces a single SRT_TTS_Studio_Portable.exe (~160 MB)
# Startup is ~3-5s slower than onedir (PyInstaller extracts to temp on each run).
# ffmpeg/ffprobe are bundled inside the exe and extracted to sys._MEIPASS at runtime.
# _check_integrity() auto-detects onefile mode and skips the integrity check.
#
# Prerequisite: run the onedir PyInstaller build first so that
#   dist\SRT_TTS_Studio\_internal\apppp_integrated*.pyd exists.

import glob, os

# Locate the compiled .pyd regardless of Python version suffix
...
_pyd_name = os.path.basename(_pyd_src)
