...
| `manga_ocr_helper.py` | voxcpm_env python (torch + **easyocr**) | **OCR ảnh truyện** for the "Dịch truyện → Tiếng Việt" feature (see manga-page section). Input JSON `["img1",...]`, output JSON `{"pages":[{"image","w","h","blocks":[{"box":[x1,y1,x2,y2],"text","conf"}]}]}` — EasyOCR line boxes are merged into bubble **blocks** (`_merge_lines`, vertical-proximity + horizontal-overlap clustering) so translated sentences read whole. `--lang` (en/chinese_sim/chinese_cht/japan/korean → EasyOCR codes), `--device auto`, `--min-conf`, `--selftest`. stdout: `PROGRESS:N:M`, `PROGRESS_MSG:..`, `WARN:..`, `ERROR:..`, `DONE:path`. **easyocr is an optional deploy-time dep** (`voxcpm_env\Scripts\python.exe -m pip install easyocr`), NOT bundled / NOT a spec hiddenimport (subprocess) — same pattern as gallery-dl/yt-dlp; only this feature errors if it's missing. The helper.py itself IS bundled (13th companion script). |
