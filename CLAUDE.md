...
**SRT autofix — `srt_autofix()` ("🔧 Sửa SRT (tự động)", `btn_srt_fix`)**: writes `<name>_fixed.srt` and reloads it. Auto-fixes: re-sorts by start time, repairs reversed end<start (end = start + QC-estimated duration), drops empty lines, nudges duplicate/regressed start times (+0.1s, strictly increasing), splits >400-char lines at the sentence boundary nearest the middle (`_srt_split_point`, each half ≥50 chars, slot divided proportionally by text length). It deliberately does NOT fix slot-overflow lines (text longer than the slot even at 2× — only shortening text helps; the report says so).
