...
**SRT time shift — `open_srt_shift_dialog()` ("⏱ Dời thời gian SRT", tools page, section `_sec_srttools`)**: shifts every timestamp of a picked SRT by ±N ms (clamped at 0) → `<name>_shifted.srt`. Note the button is created before the function is defined → `command=lambda: open_srt_shift_dialog()` (late binding).
