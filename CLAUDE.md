...
Loaded at startup via `_load_settings()`, saved via `show_settings_dialog()`. Lives next to the exe. Sibling state files live in the same directory but are deliberately **separate** (because `_save_settings()` rewrites `settings.json` wholesale): `voice_profiles.json`, `session.json`, `glossary.json`, and `ui_prefs.json` — see the Workflow-features section.
