...
|---|---|---|
| Window **X** button (`WM_DELETE_WINDOW`) | `on_app_close()` @19395 | Confirm dialog → `stop_all_processes()` → goodbye sound (`naycaugioi.wav`, sync) → `os._exit(0)`. Quits for good. |
| **Logout** button (`btn_exit`, top-right `_g6`) | `on_logout()` @19422 | Same X effect (confirm + stop processes + goodbye sound), but **relaunches** instead of exiting → returns to the login screen. |
...
`apppp_integrated.py` (~21,800 lines) is the **entire application** — no modules, packages, or separate files for UI vs logic. All TTS providers, UI, video tools, auth, and utilities are inline.
...
| ~4770–5965 | Sidebar-workspace (`# BO CUC SIDEBAR-WORKSPACE` @4793, `show_workspace` @4802, `_make_section` @4918), the **Dashboard page** build + `_dashboard_refresh`/`_db_env_check`(threaded)/`_db_preflight` (~4944–5700), then the other pages' sections, `show_fireworks` (@5969) |
| ~5965–6600 | `show_settings_dialog` (@6222) — scrollable settings dialog |
| ~6600–7820 | **Translation to Vietnamese** (LLM online + offline) — `_translate_active_key` (@6606), `_translate_segments` (@6864), `translate_srt` (@7163), `translate_pdf/doc`, `_write_translated_doc`, `_read_text_smart` |
| ~7820–12320 | Feature functions: `update_progress` (@7828), **Video OCR** (`_run_videocr_thread` @8465 + pause/stop), STT export (`_stt_export_format` @8730), **Mux** (`load_mux_video` @9301 … `start_mux_video` @9534), TTS providers (Edge/FPT/Vbee/Zalo/EverAI/MiniMax), RVC, VoxCPM, **PDF + Word/TXT TTS** (`load_pdf` @9656, `load_doc_tts` @9728), QC dashboard, compress |
| ~12320–14720 | Local voice-clone batch backends: VieNeu (`_vieneu_preflight` @12352), **F5-TTS** (`_f5tts_preflight` @12706, `_run_f5tts_batch` @12764), OmniVoice, Queue + **Auto-dubbing wizard** (`run_autodub_chain` @14143), regenerate paths |
| ~14720–18520 | `merge_ffmpeg` (@14765), **Edit Studio** (`open_edit_studio` @15868 + sequential playlist), remaining video tools, UI widget instantiation for all button rows, `_CTRL_GROUPS` / `_WS_OUTPUT` population |
| ~18520–20020 | `set_mode()` (@18592) — the central UI state machine — plus late-bound widgets (`btn_reset_mode`, Logout button), the exit / Logout flow (`on_app_close` @19395, `on_logout` @19422) |
| ~20020–end | `_run_startup_diagnostics` (@20028), splash screen (`show_splash_then_auth` @20263), `app.withdraw()` + splash scheduling, `app.mainloop()` (@20552) at module level |
...
- **Pages** (`ws_pages`, ~line 4397): one `CTkFrame` per key in `_WS_KEYS = ["dashboard","tts","voice","textaudio","doc","extract","translate","video","editstudio","tools","system"]`. `show_workspace(key)` (~4403) `pack_forget`s all pages and packs the chosen one, and highlights the active sidebar button. Sidebar items + captions come from `_WS_NAV_ITEMS` (~4419). The **default landing page is `"dashboard"`** (the `show_workspace("dashboard")` call right after all sections are built).
- **Sections (cards)** — `_make_section(title, subtitle, accent, ws=KEY, parent=None)` (~4510) creates a titled card and returns its `body` frame. By default it packs **inside `ws_pages[ws]`**; pass `parent=<frame>` to pack it into an arbitrary frame instead (used by the dashboard's collapsible diagnostics zone). Helpers `_sec_row(parent)` (horizontal) / `_sec_col(parent)` (equal-width column, `expand=True`) / `_sec_sublabel(parent, text)` build the layout inside a card. Buttons still pack `side="left", expand=True, fill="x"` for equal width within a row.
