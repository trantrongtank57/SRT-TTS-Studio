...
- **`_is_speakable(text)`** — true iff the text has ≥1 letter/digit (`[^\W_]`, Unicode). Lines that are pure punctuation/symbols/music-notes (`♪`) would only ever produce silence (`novoice`) + waste 3 retries, so the four batch entry points (`generate_tts`/`_generate_pdf_tts` sequential + `_generate_tts_parallel`/`_generate_pdf_tts_parallel` gather) **skip** them (log `⏭ … không có nội dung đọc được`, advance index, no FAIL). `_scan_missing_lines` applies the same guard so skipped lines aren't reported as missing-file FAILs.
